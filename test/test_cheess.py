...
class TestChess(unittest.TestCase):
    ...
    def setup_move_scenario(self, color, is_valid_move, move_result):
        self.chess.__tablero__.get_color = MagicMock(return_value=color)
        self.chess.__tablero__.is_valid_move = MagicMock(return_value=is_valid_move)
        self.chess.__tablero__.move_piece = MagicMock(return_value=move_result)

    def test_init(self):
        self.assertIsInstance(self.chess.__tablero__, Board)
    ...
    def test_get_captures(self):
        self.chess.__tablero__.get_capture_counts = MagicMock(return_value={"WHITE": 5, "BLACK": 3})
    ...
    def test_is_over(self):
        ...
        self.chess.__tablero__.__white_captures__ = 15
        self.assertTrue(self.chess.is_over())

        # Reset and test when black captures 15 pieces
        self.chess.__tablero__.__white_captures__ = 0
        self.chess.__tablero__.__black_captures__ = 15
        self.assertTrue(self.chess.is_over())

        # Test when king is captured
        self.chess.__tablero__.__black_captures__ = 0
        self.chess.__tablero__.__king_captured__ = True
        self.assertTrue(self.chess.is_over())

        # Test mutual agreement
        self.chess.__tablero__.__king_captured__ = False
    ...
    def test_display_board(self):
        self.chess.__tablero__.display_board = MagicMock()
        self.chess.display_board()
        self.chess.__tablero__.display_board.assert_called_once()

    def test_play_move_invalid_turn(self):
        self.chess.__tablero__.get_color = MagicMock(return_value="BLACK")
    ...
    def test_promote_pawn(self):
        self.chess.__tablero__.handle_pawn_promotion = MagicMock(return_value="QUEEN")
    ...
    def test_parse_move_invalid_non_numeric(self):
        # Caso con caracteres no numéricos (debe lanzar ValueError)
        with self.assertRaises(ValueError):
            self.chess.parse_move("a1")
        with self.assertRaises(ValueError):
